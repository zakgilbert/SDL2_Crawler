...
static double get_angle(Sprite *this)
{
    double pi = 3.14159;
...
static double get_p_angle(double angle)
{
    double dif = 0.0f;
    if (angle < 0.0f)
    {
...
static void _logic(void *obj)
{
    ...
    if (FRAMES_RENDERED % 7 == 0)
    {
        ...
        double angle = get_angle(this);
        a2 = (int)(get_p_angle(angle) / 22.0f);

        this->row_index = (a2 * this->rows) + this->col_index;
        printf("a2: %d\n", a2);
        FACING_PREV = FACING;
